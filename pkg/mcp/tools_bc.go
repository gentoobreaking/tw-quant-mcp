...
// handlerGetOtcMonthlyRevenue：上櫃公司每月營業收入彙總（T195）。
// 對稱上市 get_monthly_revenue；code 為本地端過濾（公司代號）。
func handlerGetOtcMonthlyRevenue(a *App, args map[string]any) (HandlerResult, error) {
	ctx := context.Background()
	limit, offset := listPaging(args)
	code := strVal(args["code"])
	date := a.now().Format("2006-01-02")
	rows, cached, stale, err := fetchNormalize[[]map[string]any](a, ctx,
		string(provider.TPExOtcMonthlyRev), date,
		cache.KeyString(model.SourceTPExAPI, string(provider.TPExOtcMonthlyRev), date, code, nil),
		func() ([]byte, error) { return a.fetchTPExRaw(ctx, provider.TPExOtcMonthlyRev, nil) })
	if err != nil {
		return HandlerResult{}, err
	}
	ttl, _ := a.ttlOf(cache.DatasetMonthlyRevenue) // 上櫃月營收同 monthly_revenue 政策
	lineage := postLineage(model.SourceTPExAPI, date, cached || stale, stale, ttl)
	out := make([]any, 0, len(rows))
	for _, r := range rows {
		if code != "" && rowField(r, "公司代號", "code") != code {
			continue
		}
		out = append(out, r)
	}
...
func handlerGetOtcExdividendResult(a *App, args map[string]any) (HandlerResult, error) {
	...
	date := a.now().Format("2006-01-02")
	rows, cached, stale, err := fetchNormalize[[]map[string]any](a, ctx,
		string(provider.TPExOtcExRightDay), date,
		cache.KeyString(model.SourceTPExAPI, string(provider.TPExOtcExRightDay), date, code, nil),
		func() ([]byte, error) { return a.fetchTPExRaw(ctx, provider.TPExOtcExRightDay, nil) })
	if err != nil {
		return HandlerResult{}, err
	}
	ttl, _ := a.ttlOf(cache.DatasetDailyKLine)
...
var goldSpotKinds = map[string]provider.TPExDataset{
	"spot":      provider.TPExGoldLatest,
	"highlight": provider.TPExGoldHighlight,
	"warrant":   provider.TPExGoldWarrant,
}

// handlerGetGoldSpot：黃金現貨與黃金權證（T227，tpex_gold_latest/
// market_highlight/warrant_gold）。kind 切換 spot/highlight/warrant；
// code 過濾 GoldCode 或 Code；passthrough。
func handlerGetGoldSpot(a *App, args map[string]any) (HandlerResult, error) {
	ctx := context.Background()
	limit, offset := listPaging(args)
	kindArg := strVal(args["kind"])
	if kindArg == "" {
	...
	ds, ok := goldSpotKinds[kindArg]
	if !ok {
		return HandlerResult{}, fmt.Errorf("kind 僅接受 spot/highlight/warrant，得到 %q", kindArg)
	}
	code := strVal(args["code"])
	date := a.now().Format("2006-01-02")
	rows, cached, stale, err := fetchNormalize[[]map[string]any](a, ctx,
	...
	out := make([]any, 0, len(rows))
	for _, r := range rows {
		if code != "" && rowField(r, "GoldCode", "Code") != code {
			continue
		}
		out = append(out, r)
	}
...
var otcBlockKinds = map[string]provider.TPExDataset{
	...
	"stock":       provider.TPExBlockStock,
	"summary_day": provider.TPExBlockSummaryD,
	"monthly":     provider.TPExBlockMonthly,
	"yearly":      provider.TPExBlockYearly,
}

// handlerGetOtcBlockTrade：上櫃鉅額交易系列（T225，tpex_daily_qutoes_block
// 等）。kind 切換 day/stock/summary_day/monthly/yearly；code 過濾（日/個股
// 類）；passthrough。
func handlerGetOtcBlockTrade(a *App, args map[string]any) (HandlerResult, error) {
	ctx := context.Background()
	limit, offset := listPaging(args)
	kindArg := strVal(args["kind"])
	if kindArg == "" {
	...
	ds, ok := otcBlockKinds[kindArg]
	if !ok {
		return HandlerResult{}, fmt.Errorf("kind 僅接受 day/stock/summary_day/monthly/yearly，得到 %q", kindArg)
	}
	code := strVal(args["code"])
	date := a.now().Format("2006-01-02")
	rows, cached, stale, err := fetchNormalize[[]map[string]any](a, ctx,
	...
	out := make([]any, 0, len(rows))
	for _, r := range rows {
		if code != "" && !containsAny(rowField(r, "Code", "SecuritiesCompanyCode", "StockCode"), code) {
			continue
		}
		out = append(out, r)
	}
...
// brokerTurnoverLevels 為 T224 level → 資料集對應。
