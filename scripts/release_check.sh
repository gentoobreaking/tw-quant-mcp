...
EXPECTED_TOOLS="${EXPECTED_TOOLS:-194}"
...
[ "$COUNT" = "$EXPECTED_TOOLS" ] || { echo "    ✗ 工具數 $COUNT ≠ $EXPECTED_TOOLS"; exit 1; }
[ "$HAS_SCHEMA" = "True" ] || { echo "    ✗ 工具缺 inputSchema"; exit 1; }
[ "$READONLY" = "$EXPECTED_READONLY" ] || { echo "    ✗ readOnly 數 $READONLY ≠ $EXPECTED_READONLY"; exit 1; }
