...
| 腳本 | 位置 | 用途 |
|---|---|---|
| `call_tw_quant_tools.py` | `~/Projects/tw-quant-mcp/scripts/` | stdio 子程序 + initialize 握手 + 逐一 `tools/call` 36 工具，每工具間隔 1s |
...
**Q: 想只重跑單一工具？**
A: 用 `one_tool.py`（`~/Projects/tw-quant-mcp/scripts/one_tool.py`）：
```bash
