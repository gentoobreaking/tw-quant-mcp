...
## 十、附錄：抓取 winvest 圖表方案評估（2026-08-01）

使用者提議：「讓 MCP 直接抓 winvest.tw 的圖表來顯示」。

**結論：不建議，有硬傷。**

1. **法律/條款**：winvest 的河流圖、四燈號詳細數據為 VIP 付費內容，抓圖顯示 = 繞過付費牆 + 未授權重製商業產品內容，侵權風險明確。
2. **技術脆弱**：ECharts canvas 動態渲染非靜態圖片 URL，需 headless Chrome + 登入態 + 等 JS 截圖；頁面改版即壞；高頻抓取遲早被反爬。
3. **體驗反效果**：截圖是死圖（不能 hover/縮放），比原站互動圖表更難用。

**關鍵洞察**：winvest 的數據同樣來自 TWSE/MOPS 官方公開資料（與 tw-quant-mcp 同源）。缺的不是數據，是渲染層。

**替代方向（合法、穩定、自有資料）**：
- A. 互動 HTML embed：讀 `_chart_meta` + data → 生成 ECharts HTML（與 winvest 同引擎），瀏覽器開
- B. render_ui 面板：QClaw 對話內渲染卡片/圖表
- C. 本地 HTML 報告：擴充 render_tool_snapshots.py 產出互動報告頁

## 十一、原「建議做」內容保留供參考（一至八節）
