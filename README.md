...
與 `tw-quant-daybrain`（v1.1 規格，Client 端）的契約核對結果，已獨立至
[docs/DAYBRAIN_CONTRACT_v2.1.md](docs/DAYBRAIN_CONTRACT_v2.1.md)。
重點：15 個相依工具名稱未破壞；唯一已知變更為 `derived_from` / `cache_ttl` /
`source_url` 不再輸出於正式 JSON（T021 決策，僅 debug/log 模式輸出）。
